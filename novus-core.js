...
const NovusSettings = (() => {
  ...
  const DEFAULTS = {
    userName: '', theme: 'dark', pollInterval: 10,
    hapticFeedback: true, compactMode: false,
  };

  const LIGHT_VARS = {
    '--bg':'#f8f9fb','--surface':'#ffffff','--surface-2':'#f1f3f5',
    '--surface-3':'#e5e7eb','--border':'#d1d5dc','--text':'#111827',
    '--text-2':'#374151','--text-3':'#6b7280','--accent':'#2563eb',
    '--accent-dim':'rgba(37,99,235,.07)','--green':'#059669',
    '--green-bg':'rgba(5,150,105,.08)','--red':'#dc2626',
    '--red-bg':'rgba(220,38,38,.07)','--amber':'#b45309',
    '--amber-bg':'rgba(180,83,9,.07)','--purple':'#7c3aed',
    '--purple-bg':'rgba(124,58,237,.07)','--cyan':'#0e7490',
    '--cyan-bg':'rgba(14,116,144,.07)','--fresh':'#db2777',
    '--fresh-bg':'rgba(219,39,119,.07)','--fresh-border':'rgba(219,39,119,.2)',
  };

  const DARK_VARS = {
    '--bg':'#0f1117','--surface':'#181a23','--surface-2':'#1f2230',
    '--surface-3':'#2a2d3a','--border':'#313546','--text':'#eceef4',
    '--text-2':'#9ba1b5','--text-3':'#6c7189','--accent':'#5b9aff',
    '--accent-dim':'rgba(91,154,255,.1)','--green':'#34d399',
    '--green-bg':'rgba(52,211,153,.1)','--red':'#f87171',
    '--red-bg':'rgba(248,113,113,.1)','--amber':'#fbbf24',
    '--amber-bg':'rgba(251,191,36,.1)','--purple':'#a78bfa',
    '--purple-bg':'rgba(167,139,250,.1)','--cyan':'#22d3ee',
    '--cyan-bg':'rgba(34,211,238,.1)','--fresh':'#f472b6',
    '--fresh-bg':'rgba(244,114,182,.1)','--fresh-border':'rgba(244,114,182,.25)',
  };
  ...
  function _injectModal() {
    ...
    style.textContent = `
      ...
      #novus-settings-overlay.open{display:flex;animation:ns-fade .2s ease}
      @keyframes ns-fade{from{opacity:0}to{opacity:1}}
      .ns-modal{background:var(--surface);width:100%;max-width:480px;border-radius:16px;border:1.5px solid var(--border);display:flex;flex-direction:column;max-height:90vh;overflow:hidden;animation:ns-rise .25s cubic-bezier(.34,1.56,.64,1)}
      @keyframes ns-rise{from{transform:translateY(20px);opacity:0}to{transform:translateY(0);opacity:1}}
      ...
      .ns-input{width:100%;padding:12px 14px;background:var(--surface-2);border:1.5px solid var(--border);border-radius:10px;font-size:14px;font-weight:600;color:var(--text);outline:none;font-family:inherit;transition:border-color .15s}
      ...
      .ns-danger-btn{width:100%;padding:10px;background:var(--red-bg);color:var(--red);border:1px solid rgba(248,113,113,.25);border-radius:8px;font-size:11px;font-weight:800;cursor:pointer;font-family:inherit;transition:background .15s}
      .ns-danger-btn:hover{background:rgba(248,113,113,.2)}
      .ns-footer{padding:12px 20px;border-top:1px solid var(--border)}
      .ns-save-btn{width:100%;padding:14px;background:var(--accent);color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:800;cursor:pointer;font-family:inherit;transition:filter .15s}
  ...
  function _save() {
    const nameEl = document.getElementById('ns-username');
    if (nameEl) set('userName', nameEl.value.trim());
    closeModal();
...
const NovusLoader = (() => {
  ...
  function _ensureBar() {
    ...
    const attach = () => document.body && document.body.appendChild(_bar);
  ...
  function skeletonTableRows(cols, rows = 6) {
    const shimStyle = `background:linear-gradient(90deg,var(--surface-2) 25%,var(--surface-3) 50%,var(--surface-2) 75%);background-size:200% 100%;animation:shimmer 1.4s infinite;border-radius:5px;height:12px;display:block`;
    const widths = ['60%','45%','55%','40%','50%','65%','35%','50%','40%','55%'];
  ...
  function skeletonAisleCards(count = 14) {
    return Array.from({ length: count }, () =>
      `<div class="aisle-btn" style="background:linear-gradient(90deg,var(--surface) 25%,var(--surface-2) 50%,var(--surface) 75%);background-size:200% 100%;animation:shimmer 1.4s infinite;color:transparent;pointer-events:none;min-height:72px">&nbsp;</div>`
    ).join('');
  }
